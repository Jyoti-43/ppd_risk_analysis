...
export function PPD_Risk_Analysis({
  ...
  const combinedData = (() => {
    ...
    sData.forEach((s: any) => {
      ...
      const prob =
        s.result?.risk_probability ??
        s.risk_probability ??
        s.probability ??
        s.result?.final_probability ??
        0;
    ...
    eData.forEach((e: any) => {
      ...
      dataPoints.push({
        ...
        epds: score,
        raw: e,
        id: e.result_id || e._id || e.id || e.created_at,
        type: "epds",
      });
    });
    ...
    hData.forEach((h: any) => {
      const date = new Date(
        h.created_at || h.createdAt || h.audit?.timestamp || h.date,
      );
      if (isNaN(date.getTime())) return;

      const prob =
        ((h.final_probability ??
          h.result?.final_probability ??
          h.result?.risk_probability) ||
          h.result?.probability) ??
        h.probability ??
        0;
      ...
      dataPoints.push({
        ...
        raw: h,
        id: h.id || h._id || h.result_id || h.created_at,
        type: "hybrid",
      });
    });
    ...
    if (e && e.activePayload && e.activePayload.length > 0) {
      const activeEntry =
        e.activePayload.find(
          (entry: any) =>
            (activeView === "all" || entry.dataKey === activeView) &&
            entry.value !== null,
        ) || e.activePayload[0];
  ...
  return (
    <Card className="flex flex-col border-none shadow-sm   pt-0">
      <CardHeader className="pb-4 space-y-4 pt-4 bg-primary/10 rounded-t-xl ">
        <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4">
          <div className="space-y-1">
            <CardTitle className="text-xs font-bold text-slate-400 tracking-tight uppercase">
              Risk Analysis
            </CardTitle>
            <CardDescription className="text-2xl font-black text-slate-800 leading-none">
              {activeView === "all"
                ? "Screening Trend"
                : activeView === "epds"
                  ? "EPDS Scoring"
          ...
          <div className="flex flex-wrap items-center gap-2">
            <div className="flex items-center gap-2 w-full sm:w-auto">
              <Select value={dataRange} onValueChange={setDataRange}>
                <SelectTrigger className="flex-1 sm:w-[110px] h-9 sm:h-8 text-[10px] font-bold bg-white border-slate-100 rounded-lg focus:ring-1 focus:ring-slate-200 transition-all uppercase shadow-sm">
              ...
              <Select value={activeView} onValueChange={setActiveView}>
                <SelectTrigger className="flex-1 sm:w-[140px] h-9 sm:h-8 text-[10px] font-bold bg-slate-50 border-slate-100 rounded-lg focus:ring-1 focus:ring-slate-200 transition-all uppercase shadow-sm">
        ...
        <div className="flex flex-wrap items-center gap-x-4 gap-y-2 mt-4">
          {showEpds && (
            <div className="flex items-center gap-1.5 px-2 py-1 bg-blue-50/50 rounded-md border border-blue-100/50">
              ...
              <span className="text-[10px] font-bold text-blue-700/80 uppercase tracking-tight">
                EPDS Score
              </span>
            </div>
          )}
          {showSymptom && (
            <div className="flex items-center gap-1.5 px-2 py-1 bg-amber-50/50 rounded-md border border-amber-100/50">
              ...
              <span className="text-[10px] font-bold text-amber-700/80 uppercase tracking-tight">
                Symptom %
              </span>
            </div>
          )}
      ...
      <CardContent className="h-[270px] pt-4 cursor-pointer">
          ...
          <div className="grid gap-1">
            <div className="flex items-center gap-2 leading-none font-semibold text-slate-700">
              ...
              {activeView === "all"
                ...
                : activeView === "epds"
                  ? "EPDS Score Stability"
                  : activeView === "symptom"
                    ? "Symptom Probability Map"
  ...
  if (active && payload && payload.length) {
    return (
      <div className="bg-white p-4 shadow-xl border border-slate-100 rounded-2xl min-w-[200px]">
        <p className="text-[10px] font-black text-slate-400 mb-3 uppercase tracking-[0.1em]">
          {label}
        </p>
        <div className="space-y-3">
          {payload.map((entry: any, index: number) => {
            ...
            const indicatorLabel = isEPDS
              ? "EPDS Score"
              : entry.dataKey === "symptom"
                ? "Symptom Prob."
                : "Hybrid Prob.";
